...
**Latest follow-up:** expand the map and Central Park-inspired landscape, study actual park/subway photographs, add real runners and two-way protected cycling, and remove the scene-title overlay. The header uses CitiVibe; the Field guide button is removed while help remains in Settings and the `?` shortcut. The latest density request uses 36 motor vehicles, twelve cyclists, 48 neighborhood walkers, eighteen park walkers and twelve runners. Juniper Court adds four basketball players and two pickleball players beside the basketball court. These are bounded ambient games, not an interactive sports engine. Keep the park car-free and cycle tracks on surrounding streets. The landscape and subway entrances are original compressed interpretations, not NYC GIS or copied landmarks. Finish the current work, merge latest main, resolve conflicts and push the existing PR rather than creating a duplicate; final browser/visual limitations must remain explicit.

**Court correction:** basketball is 14 x 7.5 m at (-61, 119), with rim offsets of +/-5.8 m; pickleball is a clearly smaller 6.7 x 3.05 m at (-61, 108.5), below one quarter of the basketball area. These are coherently scaled miniature footprints, not regulation facilities. Players move through drives, rebound pursuit and rally repositioning rather than remaining fixed while only their arms animate. Shared court geometry/motion stays authoritative; reduced motion keeps a still pose and the population remains unchanged.

**Merge contract:** reconcile feature checkpoint `eabf072` with main `22ec513` without dropping either feature set. Retain the 220 x 340 m map, 78 x 176 m park, 94 buildings, 36 junctions, 24 blocks, 132 people/vehicle rigs and two sports balls. Also retain main's physics-based rain/snow/wind, accumulating snow depth, rain-fed ground pools, rainfall intensity, cautious wet/snowy traffic, and standalone lower-left Settings trigger with a compact non-modal dock. Help remains modal. Preserve the Vercel cache/security-header configuration, but do not deploy. Existing feature/main test counts and scene measurements are historical pre-merge checkpoints, not verification of the merged revision. Complete integrated checks before updating the existing PR #2; this does not authorize unrelated features or broader UI redesign.
