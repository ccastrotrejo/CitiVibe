...
**Status:** the expanded park and city use asset version `rainlight-004`; integration with main's weather depth/pools is being reconciled. A large car-free Central Park-inspired landscape replaces the compact park; original neighborhood blocks, two-way protected cycling and varied traffic surround it. Authored TypeScript remains the reproducible art workflow; reference photographs remain research only. The M1-M3 and incoming-main weather measurements below are historical and do not establish final merged geometry or visual acceptance.
...
Juniper Court has an original 14 x 7.5 m basketball surface and an adjacent, clearly smaller 6.7 x 3.05 m pickleball surface, both centered on X=-61 at Z=119 and Z=108.5. Pickleball occupies 20.435 m² against basketball's 105 m², about 19.5% of its area and below one quarter. Their shared [court manifest](../src/content/courts.ts) positions rims, net, markings, players and balls consistently: basketball rim offsets are +/-5.8 m at 2.5 m above the surface; pickleball has 1.065 m kitchen depth per side and a 0.91 m net. Both surfaces remain at Y=0.04.

Four basketball rigs move through drives, passes, shots, rebound pursuit and repositioning; two paddle players move to meet the rally across the pickleball net. Motion must include readable footwork rather than merely translating an arm-animated standing pose. Palette/primitive geometry and actor instance batches are reused, with a fixed still only for reduced motion or a paused sample. These relative footprints are coherently scaled miniatures, not regulation-facility or safety-code certification, and the games are ambient choreography rather than interactive sports.
...
The subsequent six-by-six street expansion explicitly revises the CPU geometry ceiling from version 003's 200,000 to **550,000 triangles**, while tightening the visible mesh-submission ceiling from 240 to **110** and retaining **36 materials**. The additional blocks, landscape and runners must fit these bounds without removing controls or weakening collision checks. These are base source-scene accounting limits including actor artwork, not revised or verified device FPS goals. Runtime weather/snow/pool work and extra renderer passes must be measured separately, not hidden in the old counts. The feature's pre-merge checkpoint was **507,474 triangles / 88 visible submissions / 31 materials**; record new merged measurements in [acceptance criteria](ACCEPTANCE_CRITERIA.md).

There are **132 animated people/vehicles** and two sports balls. Final building and scene traversal counts belong in [acceptance criteria](ACCEPTANCE_CRITERIA.md); earlier version-002/003 counts are superseded. CPU geometry counts exclude extra renderer passes and are not FPS measurements. Shadows are cached but refresh on retained weather revisions for canopy sway and snow depth; moving actors do not cast into the cache. Precipitation allocations remain fixed while rain/cloud coverage spans the enlarged map, and unchanged actor poses are not resubmitted between simulation ticks.
