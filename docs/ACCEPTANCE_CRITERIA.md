...
**Status: the NYC realism and interior-placement slice passes integrated source and installed-Chrome checks. Physical-device and broader visual/accessibility gates remain open.** The current scene retains thirty-six intersections, twenty-four blocks, 94 buildings and a 78 x 176 m park within a 220 x 340 m map. It has 328 people including eight resting neighbors, 48 moving motor vehicles, twelve parked cars, four sidewalk food carts, three ten-bike stations and two sports balls. The occasional airplane and an occasional high-altitude balloon bunch are ambient sky decor. Full-size courts, subway entrances, scaffolding, street names, weather, lighting and existing controls remain. Previous feature and merge records below are historical. Automated coverage does not substitute for physical-device FPS, long-session, cross-combination or assistive-technology verification. Production readiness is not claimed; landmark selection, camera-follow and the drone remain excluded.

## NYC architectural character - 2026-09-17

This bounded interpretation of the existing [building-fabric research](NYC_BUILDING_FABRIC_RESEARCH.md) strengthens architectural hierarchy without enlarging the city. Fifty-six masonry/brownstone buildings retain their base bands but lose repetitive upper-floor belts; metal lofts, courtyard apartments and curtain-wall offices retain their distinct horizontal structure. Twenty-six tenements gain taller sash proportions. Those tenements and eight metal lofts receive dark projecting crowns with street-facing friezes and brackets, while brownstone principal windows gain actual side surrounds.

All 94 building IDs, lot-line joints, blank party walls, facade colors, floor levels, storefronts, stoops, murals, roof equipment, grid, park, population and controls remain. The refinement uses existing geometry/material batches without new dependencies or simulation state. It is original authored miniature art, not a traced building or an assertion that every NYC building has these details.

**Executed source checks:** strict TypeScript, whole-project ESLint, the production build and the complete single-worker suite pass: **685 passed, 14 existing skips across 55 files**, in 450.34 seconds. Three new architectural regressions failed against the original implementation. They check actual rendered floor bands, window proportions, projecting crowns, lateral lot-line clearance, upper-window/lintel clearance and brownstone side surrounds. The first implementation exposed a bracket/lintel overlap; the shortened final brackets pass without reducing the window or weakening the clearance assertion. Existing full-scene clearance, weather capture, deterministic construction, disposal, traffic, population and recovery coverage remains intact. The scoped mechanical design detector reports no findings.

Final constructed base-scene accounting is **623,774 triangles, 109 visible mesh submissions and 37 materials**, under the unchanged **<640,000 / <=110 / <=37** limits. These are CPU scene counts, not measured GPU calls, frame rates or additional weather/shadow-pass costs. The existing production large-chunk warning remains.

**Production browser:** all five selected Chromium/SwiftShader scenarios pass in 1.7 minutes on isolated port 4194: reduced-motion guided views and accessibility; nighttime rain labels; actual WebGL context restoration; repeatable paused-seed rendering without external resources; and persistent snow/wind settings through paused graphics recovery. No browser assertions, timeouts or source budgets were relaxed. Other browser scenarios were not rerun for this appearance-only pass.

**Visual boundary:** matching 1440 x 1000 development captures cover the overview, east avenue, neighborhood street wall and nighttime rain before and after, with no page errors. Only the initial east-avenue image could be pixel-inspected: the image viewer then reached its limit. Final screenshots are captured evidence, not all-angle aesthetic approval. Physical-device performance and long-session/assistive-technology sign-off remain open. The live preview is available for human inspection; no screenshot, debug route or research asset enters the runtime. The user subsequently approved the code and authorized a commit, push and PR; PR merge and deployment remain excluded.

**Base-branch sync:** the user subsequently requested merging latest `origin/main`. The branch fast-forwarded from `0a22743` to `7e3b52e` (civic density, #25), and the local architectural refinements were restored. The sole conflict was README prose; both feature descriptions are retained. Source and test refinements remain byte-for-byte unchanged from before the sync.

Post-sync TypeScript, ESLint, production build and **150 targeted tests across twelve files** pass, covering architecture, full-scene clearances/budgets, civic geometry/relocation/snow, destination access, steam, environment visuals and runtime recovery. Three served Chromium/SwiftShader scenarios also pass: actual WebGL restoration, reproducible paused rendering without external resources, and snow/wind settings through paused recovery. The combined scene measures **634,086 triangles, 109 visible mesh submissions and 37 materials**, including thirty drains, fifteen hydrants and four steam stacks; unchanged ceilings still hold. The 685-test complete run above predates this sync and is not claimed as a post-merge full-suite result. No source budget or assertion was relaxed; the existing chunk warning and visual/device limitations remain.

**Parking-outline correction:** the user's subsequent screenshot/request restores the twelve original three-sided white bay outlines, which R10 had replaced with curb-end ticks. The correction adds 36 shared box strips at the original 6.2 x 2 m reservations; parked-car poses, newer pictogram signs, drains and curb-end ticks remain. A new full-scene regression first reproduced the missing paint, then verified 108 samples across every bay's three lines above the asphalt. TypeScript, ESLint, build, all 67 targeted tests across five files, and two served Chromium/SwiftShader cases (repeatable paused rendering and snow/wind recovery) pass. The furniture-only part guard was stale after main's civic expansion: 2,168 parts before this correction, 2,204 after; its local ceiling is reconciled to 2,300. Whole-scene triangle, submission and material limits are unchanged and pass. The build's existing chunk warning remains. The user subsequently authorized committing and pushing this correction to the existing PR; no new pixel-inspection claim is made. PR merge and deployment remain excluded.

## NYC ordinary vehicle refinement - 2026-09-17

All six approved groups and the subsequent open-wheel-arch polish are implemented. **TypeScript, ESLint, the production build and all five selected served Chromium/SwiftShader scenarios pass.** The build retains its existing large-chunk warning. The PR includes genuine pre-change and final front/rear fleet images, plus the final passenger/parked-family study.
...
Final CPU traversal measures **639,571 base-scene triangles, 110 visible mesh submissions and 37 materials**, versus **638,803 / 109 / 37** before this change. Limits remain **less than 640,000 triangles, at most 110 submissions and at most 37 materials**; no unrelated scenery is simplified to fit. Taxi lettering alone saves 7,200 triangles across the nine cars before the additional shells and trim. These base measurements exclude extra weather passes and are not frame-rate evidence.

**Unit-run caveat:** the initial unrestricted full invocation reported 571 passed, 14 skipped and 14 timeout failures in existing CPU-heavy tests while other work ran. A final-source full run with `npm test -- --maxWorkers=1` reported **583 passed, 14 skipped and two timeout failures across 38 files**. The two affected tests were Juniper bike access (seed 2401, grip 0.3, walking pace 1) and full posted-stop clearance (seed 0). macOS recorded sleep intervals overlapping their abnormal 522 s and 984 s durations, including 513 s thermal-emergency sleep and 978 s maintenance sleep. Process inspection showed the runner continuing to spawn new workers after waking, not an active-computation deadlock.

Both exact unchanged scenarios passed one bounded awake retry in **37.07 s**, with other cases filtered out. All **585 runnable cases therefore passed across the full run plus targeted retry**, but this is **not an uninterrupted all-green full-suite invocation**. No timeout, safety assertion, traffic source or scene ceiling was relaxed. A temporary idle-sleep assertion was bound to the owned test processes and ended with them; thermal/system sleep was not overridden. The 14 existing skips remain unexecuted.

The independent served run passed **five tests in 3.2 minutes** on port 4200: navigation/tour interruption/pause, reduced-motion guided views and accessibility, nighttime rain labels, real WebGL context restoration, and snow/wind settings with paused recovery. Captured original fleet and live day/night-rain/snow views introduced no runtime debug route; the live capture reported no page errors. Browser and timeout-retry logs remain session artifacts. Physical-device frame times, long-session sign-off and a clean uninterrupted full-unit invocation remain unverified. The complete serial run plus exact successful retries was explicitly accepted for ready-for-review status; no further full-suite retry, merge or manual deployment was requested.
...
## NYC realism and interior placement - 2026-09-17

The [per-item implementation record](NYC_REALISM_RESEARCH_PLAN.md#local-implementation-record---2026-09-17) covers R01-R35 and R38-R39, plus the approved hospital, fire station and police station. R36-R37 remain excluded. All six mailboxes, eight street benches, nineteen small utilities and four steam stacks now occupy interior plazas, civic forecourts or spare non-cycling curb pockets, at least twelve metres inside the city boundary. Original IDs, population, parked-car poses, grid, park, full-size courts, access and controls remain.

**Civic density follow-up:** thirty street drains, fifteen hydrants and four steam stacks now occupy interior curb, sidewalk-furnishing and paved pocket locations, not the outer border ring. Drain tests keep the original grate dimensions and validate road recesses, side-aware curb mouths, snow clearing, bike-track-side exclusions and parking-bay separation. The same utility separation checks now cover every hydrant, cover, basket and steam stack against reserved furniture, station, subway, service and crossing envelopes.

Executed checks for this follow-up pass: `npm run typecheck`, `npm run lint`, the targeted eight-file Vitest run for civic utilities/integration/relocation/snow/streetscape/scene/steam/environment visuals (120 tests), and `npm run build`. The current base source-scene accounting is **633,798 triangles, 109 visible mesh submissions and 37 materials**, under the unchanged **<640,000 / ≤110 / ≤37** ceilings. The build retains the existing large-chunk warning.

**Executed source checks:** strict TypeScript, whole-project ESLint and the production build pass. The final complete single-worker suite passed **669 tests with 14 existing skips across 53 files** in 497.70 seconds, including the two-lane furnishing refinement, canonical loading-clearance reconciliation, actual-arrival service correction and complete-scene activity-clearance regressions. Source, test and public-asset fingerprints stayed unchanged throughout this final run. The two original 30-second simulation-test timeouts are retained. Scene budgets, actual body dimensions, crossing capacity and simulated waiting thresholds remain unchanged.

Coverage includes full relocated-prop and bench-user envelopes, connected civic approaches, real service-recess raycasts, natural service encounters, weather-cancelled duties, clearing a walking channel before returning, active crossing headway and landing ownership, retained pause/recovery, scheduled glazing, steam ownership, and matching cleared snow support/color/shadow fragments. All ten activity-approach variants and their entry turns are checked against the complete constructed scene. The shared (30, 104.25) service target retains a full 1.6 x 1.6 m standing/turning reservation, with raycasts over the entire pocket rather than only a smaller body proxy. A cancelled police return no longer bypasses merge holds; ordinary walkers no longer wait behind an already-vacated crosswalk departure for the entire crossing duration.

The final independent review found that a worker could complete the encounter before the approaching van arrived. Readiness now requires an actual parked dwell with at least eight seconds remaining, and the natural seed 0/2401 regressions assert the van's stop coordinates and zero speed throughout the encounter. The furnishing-clearance regression is tied to both actual lane offsets and their 7.7 m outer reservation, with full canonical bench approaches rather than an approximate user envelope.

**Production browser:** all **18 existing Playwright scenarios pass** against the final production build in 38.3 seconds using installed Chrome 153.0.8010.47 through a session-only configuration. Coverage includes camera/tour interruption, reduced motion, keyboard/accessibility checks, three desktop viewport sizes, compact Settings, no-WebGL/no-JavaScript fallbacks, shader-error reporting, real graphics restoration, retained snow/wind settings, accumulation and pause. The default software-WebGL attempt encountered startup/UI timeouts and is not claimed as a passing run. No browser assertion or timeout was relaxed.

| Observed base source-scene accounting, 09:08 UTC | Measured | Unchanged ceiling |
| --- | ---: | ---: |
| Triangles | 616,758 | <640,000 |
| Visible mesh submissions | 108 | 110 |
| Material resources | 37 | 37 |

The scene has 362 retained rigs and 94 buildings: 43 residential, 31 mixed-use, seventeen office and three civic. These are constructed source counts, not GPU draw calls, FPS or weather/shadow-pass totals. Steam adds one separate instanced effect draw with twelve 64-triangle puffs and borrows the cloud material. The build retains its large-chunk warning: renderer 455.44 kB / 121.11 kB gzip; entry 646.51 kB / 200.56 kB gzip. Optimization is not declared complete.

**Visual and reference limits:** four development views were refreshed at 1440 x 900 without page/console errors. An earlier interior-plaza image was pixel-inspected and showed furniture among building-lined pockets; the refreshed images were not inspected because of the image-viewing limit. Unavailable supplied references, default-runner limitations, all-angle art approval, physical-device FPS, long-session behavior and assistive-technology testing remain explicit open gates. The fifteen retained research artifacts are byte-for-byte unchanged. All work remains local and uncommitted; no push, pull request or deployment is authorized.
