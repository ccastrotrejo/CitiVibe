...
## Purposeful neighborhood activity - 2026-09-17

The [NYC realism implementation](NYC_REALISM_RESEARCH_PLAN.md#local-implementation-record---2026-09-17) keeps the same 328 people and 362 retained simulation rigs. Selected existing walkers physically approach, occupy and leave a storefront or bench-front reading pocket. Six park walkers have an original book and grounded reading pose. Another bounded group uses the existing meadow path with actual spline connections and reserved merges; the decorative bridge remains scenery.

The hospital, fire station and police station reuse twelve naturally adult street walkers. Their original IDs, lane directions, demographics and pace remain; they circulate their workplace blocks and reserve one off-flow pocket per facility for a 2.5-second frontage inspection and three-second quiet pause. Public doors and the closed apparatus shutter remain clear. The police pocket is west of its public door to avoid the relocated bench. Uniforms and symbols are original, unbranded and unarmed.

Existing adult `city-walker-17` visits the real service-front recess at (30, 104.25), associated with van 47's (30, 98.5) stop. A retained off-flow wait is capped at 120 seconds; the 3.2-second use clock starts only when the actual van is parked with at least eight seconds of dwell remaining. An approaching van does not count as an arrival. A missing service, adverse weather or timeout cancels the visit; van departure cancels an encounter already in use, while a visitor awaiting a known service may wait for its next arrival. Cancellation is physical, without teleportation or a stranded reservation. This is a nearby service-associated visit, not a parcel handoff across the sidewalk or an operational interior.

Approach/use/departure ownership, two-lane merge holds and existing crossing/landing/capacity rules stay in the retained simulation. Weather cancellation finishes an in-progress approach to clear the walking channel, skips the activity and waits for the return lanes to drain; a blocked approach still reverses after its existing timeout. A moving crosser releases the already-cleared departure behind them, while their full headway, downstream landing and intersection reservation remain protected. Pause and recovery do not recreate clocks, people or equipment. Reduced-motion poses remain grounded. Ordinary neighborhood travelers retain connected crossing trips; no citywide service scheduler or new population is introduced. Current verification and limitations are recorded separately in [acceptance criteria](ACCEPTANCE_CRITERIA.md).
