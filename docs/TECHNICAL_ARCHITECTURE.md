...
The architectural-character refinement reuses those construction types to distinguish masonry base/shaft/crown composition from the continuous floor bands of metal lofts, courtyard apartments and curtain-wall offices. Tenements keep their existing floor levels and window counts with taller sash proportions. `buildStreetCornice` adds street-facing friezes/brackets to tenements and metal lofts through the same borrowed box/material batches; the existing facade builder supplies the projecting roof slab and brownstone principal-window jambs. Brackets stay above the upper lintels and inside each lot's lateral span. No new resource owner, timer, actor, material or rendering path is introduced.
...
`buildBikeShare` submits fixed station parts through the district builder and persistent rigs through `ActorInstances`. It samples retained simulation time plus each station rider's `sharedBike` state before instance upload, with mutually exclusive walking/riding representations. `buildSharedBike` creates classic or electric geometry from an ID-stable `sharedBikeKind`; dock and road representations use the same ID, shared primitives/materials and contact dimensions. Electric details add no material, timer or lamp allocation. Hands use bicycle-local targets transformed into each arm parent's space; cumulative station-trip distance drives wheel/gait phase. Road-wheel pivots remain outside the suspension body and retain their 0.32 m tire radius.

Juniper's inbound and outbound crossing spans belong to its existing traffic route. `CityTraffic` retains one pending/granted access reservation rather than waiting for a huge simultaneous road/sidewalk gap at the dock. `bikeAccessStopDistance` shares expanded-body crossing bounds with `StreetPedestrians`; occupants and traffic too close to brake drain before admission, while approaching bodies hold outside. The reservation survives pause/recovery with the simulation and clears after the rider's full crossing span. It does not replace junction signals, add a scheduler or alter route geometry. Station riders already join controlled road circuits and return to their own dock; this is not a rental or arbitrary destination system.
...
The three stations retain ten-bike inventories and eleven docks each, including beside Juniper Court; a road trip transfers visibility of one bicycle rather than spawning another. The 2026-09-16 correction adds no actors. Earlier population and geometry measurements below are historical; current validation belongs in [acceptance criteria](ACCEPTANCE_CRITERIA.md).
