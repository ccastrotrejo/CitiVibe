...
`StreetPedestrians` in `pedestrians.ts` retains the 144 street actors independently of road-route randomness. It derives 72 straight crosswalk links from neighboring sidewalk corner entries/exits and precomputes block-hop distances. Each person's seeded stream chooses short block destinations, trip pace, mid-sidewalk stopping locations and brief activities. Same-block headway uses mutable block groups; crossing transfers move group membership without replacing actor/position objects. `travelDistance` is an uninterrupted gait odometer separate from route-local coordinates.
