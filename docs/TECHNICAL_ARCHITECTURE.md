...
**Status: connected-city and incoming weather/settings integration under merge verification.** The application uses React, strict TypeScript, Vite, direct Three.js, and npm with exact locked versions. Preserve the expanded street/park/court simulation, original neighborhood art, five-landmark navigation/tours, weather physics and accumulating snow/water, optional audio, compact Settings dock, quality presets, occasional airplane, and persistent local preferences. The separate pre-merge checkpoints do not establish a final merged passing result. Physical-device and long-session hardening remain, as does adaptive-quality integration/calibration. No services or backend exist; the app ships as a static Vite build (see Deployment).
...
`src/content/courts.ts` shares both court footprints, net/hoop positions and six player definitions with art and motion: 14 x 7.5 m basketball and 6.7 x 3.05 m pickleball, with the latter below one quarter of basketball's area. `CourtActivity` samples basketball drives, rebound pursuit and repositioning, plus pickleball rally coverage, from `ActorSimulation.elapsed` before actor matrices are uploaded. Players change position with readable locomotion and contact-aligned arm/ball poses. It owns no timers or GPU allocations; scene-owned meshes use the existing instance batches. Two bounded ball props and local court movement remain separate from street/park routing and intersection reservations. Rebuilding the artwork samples the retained time, while reduced motion uses a fixed still. This correction is newer than the historical feature checkpoint and needs renewed contact, spacing and lifecycle checks.
...
These shadow changes are implemented merge adaptations, not final GPU or visual verification. The expanded base-scene ceiling remains 550,000 triangles, 110 visible submissions and 36 materials. Feature checkpoint `eabf072` measured 507,474 / 88 / 31 before main's weather effects; shell/pool/particle work and extra passes require separate new measurements.
