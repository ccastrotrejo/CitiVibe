...
## NYC realism research proposal - 2026-09-16

The [real-life comparison and improvement plan](NYC_REALISM_RESEARCH_PLAN.md) records the current code baseline, source-backed NYC context, 39 prioritized items, and seven phases from target selection through integrated sign-off. Its recommendation is to pilot one coherent mixed-use blockfront before adding more detail or activity across the district. The user additionally requested street drains and steam funnels in the plan: drains belong to the street-section phase, and a bounded steam-stack/plume pilot follows clearance and effects-budget review. It preserves the current 94 buildings, 328 people, 48 moving vehicles, grid, park, courts, access, controls, and resource ceilings.

**Subsequent implementation authorization, 2026-09-16:** implement R01-R35 and R38-R39; exclude R36-R37. Also add one hospital, one fire station, and one police station with recognizable employees nearby. Existing buildings may be repurposed and building/person counts may expand if needed, but the grid, park, safe access, and rendering budgets remain fixed. These are not checked-off milestones until verified. Full-scale blocks, one-way conversion, operational transit, and skyline/waterfront additions remain excluded. No commit, push, PR, or deployment is authorized.

**Local implementation, 2026-09-17:** the [per-item record](NYC_REALISM_RESEARCH_PLAN.md#local-implementation-record---2026-09-17) now describes bounded implementations for all 37 approved items and the three civic facilities. The subsequent correction moves all six mailboxes, eight street benches, six utilities and the steam stack into interior settings with real access envelopes; no original parked car moves. Existing counts remain 94 buildings and 328 people. A small real service-front recess supports the van-associated pedestrian visit without occupying the sidewalk. Final integration checks are in progress; physical-device and broader art gates remain open, not implicitly completed by this slice.

## Latest authorized slice - connected NYC-inspired neighborhood

**2026-09-17 architectural-character refinement:** further NYC visual refinement preserves the current grid, 94 buildings, population, park and circulation. The bounded pass replaces repetitive upper-floor belts on 56 masonry/brownstone buildings with a clearer base and crown, raises tenement sash proportions, adds dark bracketed cornices to 26 tenements and eight metal lofts, and completes brownstone principal-window surrounds. It adds no skyline expansion, new simulation or broader control redesign. Source/browser evidence and the remaining image-inspection limitation belong in [acceptance criteria](ACCEPTANCE_CRITERIA.md#nyc-architectural-character---2026-09-17). The user subsequently approved the code and authorized a commit, push and PR after syncing with latest main. PR merge and deployment are not authorized.
