...
**All behavior in this document is proposed for LivingCity**, unless explicitly called a source observation. These are working defaults for a future implementation, not recovered source algorithms or completed features. [Product scope](PRODUCT_BRIEF.md) takes precedence; the [reference analysis](REFERENCE_ANALYSIS.md) records what was actually observed.
...
| --- | --- |
| Renderer lifecycle | Loading, ready, load error, unsupported WebGL, context lost, restoring, static fallback, disposed. |
| Camera mode | `overview`, `free`, `focus`, `follow`, `tour`. |
| Selection | An original POI ID, an actor ID, or none; independent of camera motion. |
...
Use one camera controller with bounded pan, zoom, height/tilt, and near/far distances. The first slice uses an orthographic projection as a proposal. World bounds and target anchors determine limits; no camera clipping through buildings or flying indefinitely off the district.

| Mode | Meaning | Entry | Exit |
| --- | --- | --- | --- |
| `overview` | Stationary framing of the original district. | Initial ready state or Reset. | Manual input, focus, follow, or tour. |
| `free` | Viewer-controlled pose, stationary without input after optional short damping. | Manual navigation or Stop automatic view. | Focus, follow, tour, Reset. |
| `focus` | Fixed composition around one original landmark; transition then hold. | Landmark selection or Focus action. | Manual navigation, another target, follow, tour, Reset. |
| `follow` | Camera tracks a selected ordinary bus/drone at a bounded offset. | Explicit follow action while running. | Manual navigation, Stop, target loss, another mode, Reset. |
| `tour` | Contextual sequence of composed views; only this controller drives it. | Explicit Start tour while running. | Manual navigation, Stop, new selection/follow, Reset. |

**Arbitration:** commands are serialized; a new user camera command cancels the previous transition before taking ownership. Pan, wheel/pinch zoom, rotate, and their button/keyboard equivalents immediately cancel follow/tour, enter `free`, and start from the current rendered pose. They do not clear the selection. Do not restart automation when input ends or when a panel closes.

Selecting a landmark cancels automation, sets the selection, and enters `focus`. Selecting a moving actor cancels existing automation, enters `free` at the current pose, and shows its short status and available Follow action; it does not silently start following. A dedicated "Follow bus" or "Follow drone" control explicitly selects and follows a valid actor. Repeated commands must not create stacked tweens.
...
Follow resolves an actor by stable ID, looks slightly ahead along its route, and clamps camera height and distance. Smooth ordinary route turns without snapping at route joins. Keep the followed actor allocated across quality changes. If it is removed, cannot be resolved, or cannot be followed safely, stop at the current view and announce "Follow ended: target unavailable." Do not silently choose a different target. Selecting Follow with no suitable actors reports why it is unavailable.

Global tour mixes overview/high angles, restrained street-level glides within safe anchors, brief original-landmark holds, and optional short bus/drone follow segments. Suggested segments are 12-20 seconds, with 4-8 second holds; timing and choices are seeded for reproducibility. Skip unavailable segments with a bounded fallback to a known valid static anchor; stop with a visible message if no segment is valid.

Starting tour with a selected landmark uses a slow bounded orbit of that landmark. With a selected actor it begins with that actor's follow segment, then proceeds through the global route. Tour status identifies the current subject and always exposes Stop. With reduced motion, replace automatic travel/advance with **stepwise guided views** and explicit Previous/Next actions. Guided views use ordinary `focus` mode without an active tour clock and remain usable while paused.
...
Opening settings/help suspends camera automation and disables scene input while the panel is modal; world actors continue unless separately paused. Closing the panel leaves the camera stationary in `free` mode and offers an explicit Resume tour/Resume follow action if applicable. That action starts from the current pose and revalidates the target. There is no unsolicited camera movement after dismissing UI.

Selecting a new mode or using manual navigation discards that resumable camera intent. User pause, page hiding, and context loss instead retain valid follow/tour intent for the resume rules below. A modal-open event must not overwrite an existing user pause.
...
| Autonomous camera motion and decorative animation | Freeze pose/progress, including in-flight focus/reset transitions and tour/follow; clear manual inertia. | Continue preserved valid mode/transition, unless a new camera command or modal dismissal canceled it. |
...
Start continuous tour/new follow is unavailable while paused, with a visible explanation to resume the city first. Existing follow/tour can remain suspended. Stop, Reset, manual navigation, settings, landmark focus, and reduced-motion guided views still work. The main pause control uses a meaningful label/state such as "Pause city" / "Resume city"; do not communicate pause through an icon alone.
...
| Quality | Automatic, High, Lightweight. High is not unbounded native DPR. Automatic reports its effective tier and adjusts gradually based on local measurements, with hysteresis. Lightweight reduces effects/density, not essential controls or valid follow targets. |
...
On the scene, one-finger drag pans, pinch zooms, and a deliberate two-finger sideways gesture rotates. Mouse primary drag pans, secondary drag rotates, wheel zooms, and Shift-wheel rotates. Lock the gesture after an intent threshold; do not alternate between pinch and rotation mid-gesture. Safari-specific twist is not required; explicit rotate buttons are the reliable fallback.
...
| `B` / `D` | Follow an ordinary bus/drone, if running and available. |
...
Honor `prefers-reduced-motion` by default, with an explicit local preference override. Entering reduced mode pauses the world, cancels inertia and automatic camera motion, and exposes a clear Resume action. The viewer may opt into conservative actor movement and explicit follow; disable decorative bobbing, camera lag/overshoot, fast particles, and automatic tour travel. Reduced-mode follow tracks a stable actor offset without cinematic lag. Explicit focus/reset and guided-view changes are immediate.
...
| --- | --- |
| Asset/network failure | Clear reason where known; Retry assets and Use static view. No endless spinner. |
| Unsupported/disabled WebGL | Explain reduced rendering capability; show the static experience immediately. |
...
| Fallback active | Label it Static view. Disable unavailable follow/tour with explanations, not silent no-ops. Keep theme/time depiction and landmark focus meaningful where supported. |
