...
Three blue bike stations each contain ten bicycles docked side by side in parallel, with eleven docks. One occupies the apron beside Juniper Court and two occupy clear inter-building pockets. Dephased checkout/check/redock vignettes use persistent adults and bicycles, sampled from retained simulation time. People walk their bicycles 0.8 m within the dedicated bay; the twelve road cyclists ride matching models on controlled routes. This does not simulate station-to-station rental journeys. Reduced motion holds station activity still, and restoration recreates the same pose and inventory.
