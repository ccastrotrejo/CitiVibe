...
export function buildVehicleRig(
  ...
  const buildWheel = (
    ...
    spin.add(limb(cylinder, palette.stone, [Math.sign(x) * (emergency ? 0.06 : 0.1), 0, 0],
  ...
  const length = service?.length ?? (busType ? 4.6 : type === 'truck' ? 4.5 : van ? 3.3 : 2.7);
  const width = service?.width ?? (busType ? 1.9 : large ? 1.7 : van ? 1.5 : 1.45);
  const radius = service?.radius ?? (large ? 0.36 : 0.26);
  ...
  for (const side of [-1, 1]) {
    lamp('head', side * width * 0.3, service?.lampY ?? 0.75, length / 2 - 0.005, [0.24, 0.18, 0.065]);
    ...
    for (const z of [-length * 0.32, length * 0.32]) {
      wheels.push(buildWheel(group, side * (width / 2 - (emergency ? 0.07 : 0.13)),
