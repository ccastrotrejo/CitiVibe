...
describe('bounded city lighting renderer', () => {
  ...
  it('draws all 48 vehicle rigs in one lens batch and all night road pools in one batch', () => {
    const world = fixture();
    world.draw();
    expect(world.batch('Soft ground illumination').count).toBe(0);
    expect(world.batch('Vehicle lamp lenses').count).toBe(36 * 11 + 12 * 2);
    world.environment.setTime('night', DATE);
    world.draw();
    expect(world.batch('Soft ground illumination').count).toBe(STREET_LAMPS.length + PARK_LAMPS.length + COURT_LAMPS.length + 48 * 2);
