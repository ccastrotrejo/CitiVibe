...
describe('versioned local preferences', () => {
  it('uses defaults without writing storage or restoring sound consent', () => {
    ...
    expect(DEFAULT_PREFERENCES).toMatchObject({ weather: 'sunny', timeMode: 'afternoon', natural: false, muted: true });
  ...
  it.each(WEATHER_MODES)('round trips v1 %s weather using only allowlisted preferences', (weather) => {
    const next = { ...DEFAULT_PREFERENCES, weather, motion: 'reduced' as const, muted: false };
    const setItem = vi.fn();
  ...
  it('reads older v1 preferences without rain intensity while preserving their other choices', () => {
    ...
    const saved = { ...legacy, weather: 'snow', natural: true, volume: 0.7, muted: false };
