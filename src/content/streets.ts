export const STREET_X = [-60, -30, 30, 60] as const;
export const STREET_Z = [-54, -26, 26, 54] as const;
...
export const CITY_EXTENT = { x: 68, z: 62 } as const;
...
export const STREET_BLOCKS = STREET_Z.slice(0, -1).flatMap((minZ, row) =>
  STREET_X.slice(0, -1).flatMap((minX, column) => column === 1 && row === 1 ? [] : [{
    id: `block-${column}-${row}`,
    minX,
...
export const TRAFFIC_ACTORS: readonly TrafficActorDefinition[] = Object.freeze([
  ...Array.from({ length: 24 }, (_, index): TrafficActorDefinition => {
