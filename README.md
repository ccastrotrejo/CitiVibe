...
Three unbranded blue bike stations each hold **ten bicycles in a parallel, side-by-side row**, with eleven docks per station. One station sits beside Juniper Court; two occupy clear inter-building pockets instead of the former narrow island-edge strips. A neighbor visibly unlocks a bike, walks it 0.8 m out, checks it and returns/relocks it; the same ten bicycles remain visible throughout. The twelve signal-controlled road cyclists use matching blue step-through frames, baskets and fenders. Station activity is a bounded checkout/redock vignette, **not station-to-station trips or sidewalk riding**.
