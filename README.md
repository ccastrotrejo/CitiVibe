...
**Verification boundary:** earlier merge results are historical, not verification of later changes. The emergency-vehicle pass successfully ran served Chromium checks for navigation/pause, reduced motion, nighttime rain and snow/context recovery, superseding the earlier startup failures for those scenarios. Current results and limitations are recorded in [acceptance criteria](docs/ACCEPTANCE_CRITERIA.md). Physical-device frame-rate and long-session sign-off remain outstanding.
